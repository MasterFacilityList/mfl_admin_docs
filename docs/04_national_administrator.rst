...
The ``Add Ward`` button is to the top right for adding a new ward.
...
``Town link`` on the sidebar. This lists all towns in a grid.
To the top right is the ``New Town`` button enabling a user add a new town.
...
Below is the next view after a user clicks the ``New Town`` button,
...
the National Administrator clicks the ``System Setup link`` on the
...
To the top right of services listing view is the ``Add Service`` button.
...
On clicking of the ``Add Service link`` the view below is brought up, consisting of
...
+++++++++++++++++++++++++++++++++
Contact types for all contacts (users, facilities e.t.c) are centrally managed here. They are
available at the contact types section of the system setup.
...
++++++++++++++++++++++++++++++++++++++++
Departments are the various sections in a facility that may have different regulation
